...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double *px, cf, tdres, reptime, cohc, cihc, dc;
	int    nrep, pxbins, lp, totalstim, species;
    mwSize outsize[2];
    
	double *pxtmp, *cftmp, *nreptmp, *tdrestmp, *reptimetmp, *cohctmp, *cihctmp, *speciestmp, *dctmp;
    double *ihcout;
   
	void   IHCAN(double *, double, int, double, int, double, double, int, double *,double);
...
dctmp = mxGetPr(prhs[8]);
	...
	IHCAN(px,cf,nrep,tdres,totalstim,cohc,cihc,species,ihcout,dc);
...
void IHCAN(double *px, double cf, int nrep, double tdres, int totalstim,
                double cohc, double cihc, int species, double *ihcout, double dc)
{	
    ...
    double NLogarithm(double, double, double, double,double);
    ...
    {    
        ...
        c1vihctmp  = NLogarithm(cihc*c1filterouttmp,0.1,ihcasym,cf,0);
	     
		c2vihctmp = -NLogarithm(c2filterouttmp*fabs(c2filterouttmp)*cf/10*cf/2e3,0.2,1.0,cf,0); /* C2 transduction output */


            
        ihcouttmp[n] = IhcLowPass(c1vihctmp+c2vihctmp-dc,tdres,3000,n,1.0,7);
   };  /* End of the loop */
...
{    
	double R,dc1,R1,s0,x1,out,minR;
    ...
    dc1 = (asym-1)/(asym+1.0)/2.0-minR;
    R1 = R-minR;

    /* This is for new nonlinearity */
    s0 = -dc1/log(R1/(1-minR));
...
double NLogarithm(double x, double slope, double asym, double cf,double dc)
{
